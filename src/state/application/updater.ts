...
import { supportedRouterName } from 'utils/supportedRouterName'
import { switchToNetwork } from 'utils/switchToNetwork'

import { setImplements3085, updateBlockNumber, updateChainId, updateRouterName } from './reducer'
...
export default function Updater(): null {
  ...
  useEffect(() => {
    const chainId = debouncedState.chainId
    dispatch(updateChainId({ chainId: chainId ? supportedChainId(chainId) ?? null : null }))
    dispatch(updateRouterName({ routerName: supportedRouterName(chainId || SupportedChainId.BSC) }))
