...
import AutonomyHistory from 'components/Autonomy/AutonomyHistory'
// import { PoweredBy } from 'components/Autonomy/PoweredBy'
import RateInputPanel from 'components/Autonomy/RateInputPanel'
import { LoadingOpacityContainer } from 'components/Loader/styled'
import { NetworkAlert } from 'components/NetworkAlert/NetworkAlert'
import { AdvancedSwapDetails } from 'components/swap/AdvancedSwapDetails'
import { AutoRouterLogo } from 'components/swap/RouterLabel'
import { ConditionText } from 'components/swap/styleds'
...
import { useAutoSwapCallback } from '../../hooks/useSwapCallback'
...
export default function StopLoss({ history }: RouteComponentProps) {
  ...
  const { callback: swapCallback, error: swapCallbackError } = useAutoSwapCallback(
    trade,
    allowedSlippage,
    recipient,
    signatureData,
  ...
  return (
    <>
      ...
      <NetworkAlert />
      <AppBody>
        <SwapHeader title="Stop Loss" allowedSlippage={allowedSlippage} />
        <Wrapper id="stop-loss-page">
          <ConfirmSwapModal
            isOpen={showConfirm}
            type="Stop Loss"
            trade={trade}
          ...
          <AutoColumn gap={'sm'}>
            <CurrencyInputPanel
              label={independentField === Field.OUTPUT && !showWrap ? <Trans>From (est.)</Trans> : <Trans>From</Trans>}
            ...
            <RateInputPanel
              ...
              onUserInput={(value) => onStopLossValuesChange('price', value)}
              id="stop-loss-currency-rate"
                ...
                <RowFixed>
                  <LoadingOpacityContainer $loading={routeIsSyncing}>
                    ...
                    <AutoRow justify="flex-end">
                      <ConditionText severity={stopLossMarketStats > 0 ? 0 : stopLossMarketStats === 0 ? 1 : 4}>
                        {Math.abs(stopLossMarketStats).toFixed(2)}% {stopLossMarketStats > 0 ? 'below' : 'above'} market
                        value
                      </ConditionText>
                    </AutoRow>
                  </LoadingOpacityContainer>
            ...
            {/* <Row justify="center">
              <PoweredBy>Powered by Autonomy Network</PoweredBy>
            </Row> */}
            <div>
              {swapIsUnsupported ? (
                      ...
                      <AutoRow justify="space-between" style={{ flexWrap: 'nowrap' }}>
                        <span style={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          {approvalState === ApprovalState.APPROVED || signatureState === UseERC20PermitState.SIGNED ? (
                            ...
                            <Trans>Allow AutoSwap to use your {currencies[Field.INPUT]?.symbol}</Trans>
                          )}
                        </span>
                        {approvalState === ApprovalState.PENDING ? (
                          <Loader stroke="white" />
                ...
                <ButtonError
                  ...
                  id="swap-button"
                  disabled={!isValid || priceImpactTooHigh || !!swapCallbackError || stopLossMarketStats <= 0}
