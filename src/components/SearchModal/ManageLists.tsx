...
const ListRow = memo(function ListRow({ listUrl }: { listUrl: string }) {
  ...
  const { current: list, pendingUpdate: pending } = listsByUrl[chainId][listUrl]
  ...
  }, [dispatch, listUrl, pending, chainId])
  ...
  const handleRemoveList = useCallback(() => {
    ...
    if (window.prompt(t`Please confirm you would like to remove this list by typing REMOVE`) === `REMOVE`) {
      ...
      dispatch(removeList({ url: listUrl, chainId }))
    }
  }, [dispatch, listUrl, chainId])
  ...
  const handleEnableList = useCallback(() => {
    ...
    dispatch(enableList({ url: listUrl, chainId }))
  }, [dispatch, listUrl, chainId])
  ...
  const handleDisableList = useCallback(() => {
    ...
    dispatch(disableList({ url: listUrl, chainId }))
  }, [dispatch, listUrl, chainId])
...
export function ManageLists({
  ...
  const tokenCountByListName = useMemo<Record<string, number>>(
    () =>
      Object.values(lists[chainId]).reduce((acc, { current: list }) => {
        if (!list) {
          return acc
        }
        return {
  ...
  const sortedLists = useMemo(() => {
    const listUrls = Object.keys(lists[chainId])
    return listUrls
      .filter((listUrl) => {
        // only show loaded lists, hide unsupported lists
        return Boolean(lists[chainId][listUrl]?.current) && !Boolean(UNSUPPORTED_LIST_URLS.includes(listUrl))
      })
      .sort((listUrlA, listUrlB) => {
        const { current: listA } = lists[chainId][listUrlA]
        const { current: listB } = lists[chainId][listUrlB]
  ...
  }, [chainId, lists, activeListUrls, tokenCountByListName])
