...
import { ExtendedEther, NativeToken, WETH9_EXTENDED } from '../constants/tokens'
...
export function useUnsupportedTokens(): { [address: string]: Token } {
  ...
  const l2InferredBlockedTokens: typeof unsupportedTokens = useMemo(() => {
    ...
    const { current: list } = listsByUrl[chainId][listUrl]
    if (!list) {
      return {}
    }
...
export function useSearchInactiveTokenLists(search: string | undefined, minResults = 10): WrappedTokenInfo[] {
  ...
  return useMemo(() => {
    ...
    for (const url of inactiveUrls) {
      const list = lists[chainId][url]?.current
      if (!list) continue
...
export function useCurrency(currencyId: string | null | undefined): Currency | null | undefined {
  ...
  const isNativeCurrency = isETH || isBNB || isAVAX
